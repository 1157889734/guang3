...
int32_t DrvUART_Write(UART_PORT	u16Port,uint8_t	*pu8TxBuf, 	uint32_t u32WriteBytes)
{
	...
	if(u16Port == UART_PORT0)
	{
	    ...
	    {
		   UART0->DATA = pu8TxBuf[u32Count];						/* Send UART Data from buffer */	// 2014-01-03修改，从下面移到上面，否则输出会有时多发一字节0x00
	       u32delayno = 0;
		   while (UART0->FSR.TX_EMPTY !=1)	// (UART0->ISR.THRE_IF != 1)		// (UART0->FSR.TX_EMPTY !=1)							/* Wait Tx empty and Time-out manner */
	       {
	       	    u32delayno++;
	       	    if ( u32delayno >= 0x40000000 )
	       	       return E_DRVUART_ERR_TIMEOUT;
