...
int main(void)
{
    ...
    */
    while (1)
    {
        ...
        if (check_timer(&run_timer, 300)) //运行灯
        {
...
#endif
#if CHOISE_PTU
        DispFun_DS();   //广3 广4 PTU       //有开门侧信息，紧急报警可以显示，没有广告
#else
        DispFun_NDS();  //伊朗PTU         在LED点阵里面显示开门侧信息   适合伊朗PTU，可添加广告信息
#endif
#if 1
        ...
        if (gScrSavTime > (NO_PROCESS_TIME ) * 20 || (gScrSavTick > (NO_PROCESS_TIME ) * 300))                          /* 约20分钟*/
        {
        ...
        if (gErrorTime > ((NO_PROCESS_TIME ) * 150) )                           /*约3分钟*/
        {
            gErrorTime = 0;
