...
/** @defgroup Flash_Latency 
  * @{
  */
typedef struct dir_play	
{ 					
   	uint8_t	start;				// 显示首项				   
   	uint8_t	cmd;				// 显示命令				   
   	uint8_t	mode1;				// 显示方式			   		   
   	uint8_t color;				// 显示颜色			   			   
   	uint16_t length1;			// 显示长度
}dir_play; 
#define Terminateled//cabinet//
#ifdef cabinet
#define         DeviceRows                      160//列*9
#define         ShiftRows                      180
#define         Devicetype                      0xf6
#else
#define  	DeviceRows                 	96// 列*9
#define         ShiftRows                       108
#define         Devicetype                      0xf7
#endif
#define 	MAX_DISP_WIDE				1024	//屏幕最大的显示宽度是1024个点
#define		MAX_BUF_SIZE				MAX_DISP_WIDE*2	//2048字节
//加上两个DeviceRows是因为：图像宽度大于DeviceRows时，缓冲区开头要插入一屏空白的数据，
//结尾又要插入一屏空白数据。这是跟显示效果有关的。
#define  	DISP_BUF_SIZE               MAX_DISP_WIDE*2 + DeviceRows + DeviceRows
