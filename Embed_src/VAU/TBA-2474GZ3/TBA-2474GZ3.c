...
unsigned char data base_volumn = 6;			// 2015-01-09修改 3;								// 2014-04-02修改，volume_AGC_val也需修改为volume_AGC_val[2] = {3, 0};							unsigned char data base_volumn = 6;					// 基础音量值为6

// 设定噪声采样值范围，采样值低于92，音量值不作调整
// 采样值大于90小于150，增益为6
// 采样值大于150，增益为3 + 6 = 9
unsigned char noise_ad_val[2] = {40, 50};					//{90, 150};
unsigned char volume_AGC_val[2] = {3, 3};		// 2015-01-09修改{3, 0};						// unsigned char volume_AGC_val[2] = {3, 3};
...
void get_ver_info(void)
{
	...
	put_n_char("\t版本信息:  Ver 1.2  2015-01-09\r\n", 33);
