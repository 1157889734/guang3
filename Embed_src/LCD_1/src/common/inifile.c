	...
	*sec_e = *sec_s = *key_e = *key_s = *value_s = *value_e = -1;
	...
	while( !end_of_string(p[i]) ) {
		...
		if( ( 0==i ||  newline(p[i-1]) ) && left_barce(p[i]) )
		{
			...
			//LYQ	增加长度限制，保证完全一致才可以!
			if((strlen(section)==(i-section_start)) && 0 == strncmp(p+section_start,section, i-section_start)) 
			{
			//printf("section size: %s, %d------%d\n",section, strlen(section), i-section_start);
				int newline_start=0;
...
int read_profile_string_from_buf( const char *section, const char *key,char *value, 
		 int size, const char *default_value, char *file_buf)
{
	int sec_s,sec_e,key_s,key_e, value_s, value_e;
	...
	assert(value != NULL);
	assert(size > 0);
	assert(file_buf !=NULL &&strlen(key));

	if(!parse_file(section,key,file_buf,&sec_s,&sec_e,&key_s,&key_e,&value_s,&value_e))
	{
		if(default_value!=NULL)
		{
			strncpy(value,default_value, size);
	...
	else
	{
		...
		memset(value, 0, size);
		memcpy(value,file_buf+value_s, cpcount );
		value[cpcount] = '\0';
				...
				const char *file)
{
	...
	{
#if PRINT_INFO
		printf("%s->%s->%s is default=%d\n", file, section, key, default_value);
#endif
	...
	{
#if PRINT_INFO
		printf("%s->%s->%s is read_value=%d\n", file, section, key, atoi(value));
#endif
		return atoi(value);
	}
}
...
*@return profile int value,if read fail, return default value
*/
int read_profile_int_16( const char *section, const char *key,int default_value, 
				const char *file)
{
	char value[32] = {0};
	...
	{
#if PRINT_INFO
		printf("%s->%s->%s  is default=0x%x\n", file, section, key, (unsigned int)default_value);
#endif
		return default_value;
	}
	else
	{
#if PRINT_INFO
		printf("%s->%s->%s  is read_value=0x%x\n", file, section, key, (unsigned int)strtol(value, 0, 16));
#endif
		return strtol(value, 0, 16);
	}
}
...
void read_profile_char_from_buf( const char *section, const char *key,int default_value, char *read_info, char *buf)
{
	...
	if(!read_profile_string_from_buf(section,key,info, sizeof(info),NULL,buf))
	{
#if PRINT_INFO
		printf("%s->%s is default=%d\n", section, key, default_value);
#endif		
		return;
	}
	else
	{
#if PRINT_INFO
		printf("%s->%s is read_value=%s\n", section, key, info);
#endif
		strcpy(read_info, info);
		return;
	}
}
...
int read_profile_int_from_buf( const char *section, const char *key,int default_value, char *buf)
{
	...
	if(!read_profile_string_from_buf(section,key,value, sizeof(value),NULL,buf))
	{
#if PRINT_INFO
		printf("%s->%s is default=%d\n", section, key, default_value);
#endif
...
int read_profile_int_16_from_buf( const char *section, const char *key,int default_value, char *buf)
{
	...
	{
#if PRINT_INFO
		printf("%s->%s is read_value=0x%x\n", section, key, (unsigned int)strtol(value, 0, 16));
#endif
		return strtol(value, 0, 16);
	}
}
