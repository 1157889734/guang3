...
typedef enum
{
    ...
    COLOR_FMT_BUTT       /*other frm which not support*/
} COLOR_FMT_E;

typedef struct
{
    COLOR_FMT_E         enFmt;            /*color fmt */

    HI_S32              s32IsFrameValid; /*1: frame is valid 1 and output to vo,0: frame is invalid,mark to idle*/
    ...
    HI_S32              s32CbPhyAddr;    /* interlace color addr when fmt is semi-planar  or Cb address when frmt is plannar */
    HI_S32              s32CrPhyAddr;    /* invalid when fmt is semi-planar,or Cr address when fmt is planar */
