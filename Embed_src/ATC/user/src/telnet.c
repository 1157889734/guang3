...
extern uint8_t send_data_to_net_flag;// added by Lison on 2012-8-27 10:40:32 
...
static err_t telnet_recv(void *arg, struct tcp_pcb *pcb, struct pbuf *p, err_t err)
{
  ...
  else if (err == ERR_OK) 
  {
    ...
    mem_free(name);
    pcb23 = NULL;// added by Lison on 2012-8-24 16:32:27 
    send_data_to_net_flag = 0;// added by Lison on 2012-8-27 10:41:33 
...
{     
  ...
  if(pcb23 != NULL)
  {
      ...
      tcp_close(pcb23);// added by Lison on 2012-8-29 11:43:26 
      //tcp_abort(pcb23);
  }
  pcb23 = pcb;
  return ERR_OK;
...
void send_msg_to_telnet(unsigned char *buf, int length)
{
    if(NULL != pcb23)
   {
	tcp_write(pcb23, buf, length, TCP_WRITE_FLAG_COPY);//TCP_WRITE_FLAG_MORE);
