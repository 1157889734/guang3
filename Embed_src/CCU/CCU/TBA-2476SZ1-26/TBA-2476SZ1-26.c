...
void pa_2_link_pa_on(void);							// PA到LINK_PA通道打开					
void pa_2_link_pa_off(void);						// PA到LINK_PA通道关闭
void pc_2_link_pc_on(void);							// PC到LINK_PC通道打开
void pc_2_link_pc_off(void);						// PC到LINK_PC通道关闭
void link_pa_2_pa_on(void);							// LINK_PA到PA通道打开
void link_pa_2_pa_off(void);						// LINK_PA到PA通道关闭
void link_pc_2_pc_on(void);							// LINK_PC到PC通道打开
void link_pc_2_pc_off(void);						// LINK_PC到PC通道关闭
...
void pa_2_link_pa(void)
{
	if(work_status == IDLE)							// 工作在空闲状态
	{
		if(pa_state_chk() ==  1)					// 检测到PA总线电平拉低，PA_STATE_CHK = 0
		{
			...
			if((link_pa_state_chk() == 0) && (link_pc_state_chk() == 0))	// LINK_PA、LINK_PC状态线都没有拉低，则进入PA到LINK_PA状态
			{
				pa_2_link_pa_on();					// PA到LINK_PA通道打开

				LINK_PA_LED = ON;					// LINK_PA指示灯点亮

				debug_printf("work_status = PA_2_LINK_PA_SUCCESS\r\n", 36);
				work_status = PA_2_LINK_PA_SUCCESS;	// 工作状态为PA到LINK_PA
			}
		}
	}
	else if(work_status == PA_2_LINK_PA_SUCCESS) 	// 工作状态为PA到LINK_PA
	{
		pa_led_blink(50);							// PA指示灯间隔500ms闪亮

		if(pa_state_chk() == 0)						// 检测到PA总线电平抬高，PA_STATE_CHK = 1
		{
			debug_printf("PA_STATE_CHK = 1\r\n", 18);
			debug_printf("work_status = PA_2_LINK_PA_SUCCESS\r\n", 36);

			pa_2_link_pa_off();						// PA到LINK_PA通道关闭
			
			PA_LED = OFF;							// PA指示灯熄灭		
			LINK_PA_LED = OFF;						// LINK_PA指示灯熄灭

			debug_printf("work_status = IDLE\r\n\r\n\r\n", 24);
			work_status = IDLE;						// 工作状态为IDLE
		}			
	}	
}
...
void pc_2_link_pc(void)
{
	if((work_status == IDLE) || (work_status == PA_2_LINK_PA_SUCCESS) || (work_status == LINK_PA_2_PA_SUCCESS))		// 工作在空闲、PA到LINK_PA、LINK_PA到PA状态， PC优先级高于PA
	{
		if(pc_state_chk() == 1)						// 检测到PC总线电平拉低，PC_STATE_CHK= 0
		{
			...
			if(work_status == PA_2_LINK_PA_SUCCESS)	// 工作在PA到LINK_PA状态
			{
				pa_2_link_pa_off();					// PA到LINK_PA通道关闭
				PA_LED = OFF;						// PA指示灯熄灭
				LINK_PA_LED = OFF;					// LINK_PA指示灯熄灭				
			...
			if(work_status == LINK_PA_2_PA_SUCCESS)	// 工作在LINK_PA到PA状态
			{
				link_pa_2_pa_off();					// LINK_PA到PA通道关闭
				LINK_PA_LED = OFF;					// LINK_PA指示灯熄灭
				PA_LED = OFF;						// PA指示灯熄灭				
			}
			
			if(link_pc_state_chk() == 0)			// LINK_PC状态线没有拉低，则进入PC到LINK_PC状态
			{
				pc_2_link_pc_on();					// PC到LINK_PC通道打开
				
				LINK_PC_LED = ON;					// LINK_PC指示灯点亮
				
				debug_printf("work_status = PC_2_LINK_PC_SUCCESS\r\n", 36);
				work_status = PC_2_LINK_PC_SUCCESS;	// 工作状态处于PC到LINK_PC状态	
			}				
		}
	}
	else if(work_status == PC_2_LINK_PC_SUCCESS)	// 工作状态为PC到LINK_PC
	{
		pc_led_blink(50);							// PC指示灯间隔500ms闪亮

		if(pc_state_chk() == 0)						// 检测到PC总线电平抬高，PC_STATE_CHK = 1
		{
			debug_printf("PC_STATE_CHK = 1\r\n", 18);
			debug_printf("work_status = PC_2_LINK_PC_SUCCESS\r\n", 36);

			pc_2_link_pc_off();						// PC到LINK_PC通道关闭

			PC_LED = OFF;							// PC指示灯熄灭
			LINK_PC_LED = OFF;						// LINK_PC指示灯熄灭
		...
		if(pa_state_chk() ==  1)					// 检测到PA总线电平拉低，PA_STATE_CHK = 0
			PA_LED = ON;							// PA指示灯点亮
		else if(pa_state_chk() ==  0)				// 检测到PA总线电平抬高，PA_STATE_CHK = 1
			PA_LED = OFF;							// PA指示灯熄灭

		if(link_pa_state_chk() == 1)				// 检测到LINK_PA电平拉低，LINK_PA_STATE_CHK = 0
			LINK_PA_LED = ON;						// LINK_PA指示灯点亮
		else if(link_pa_state_chk() ==  0)			// 检测到LINK_PA电平抬高，LINK_PA_STATE_CHK = 1
			LINK_PA_LED = OFF;						// LINK_PA指示灯熄灭			
	} 
}
...
void link_pa_2_pa(void)
{
	if(work_status == IDLE)							// 工作在空闲状态
	{
		if(link_pa_state_chk() == 1)				// 检测到LINK_PA状态拉低，LINK_PA_STATE_CHK = 0
		{
			debug_printf("LINK_PA_STATE_CHK = 0\r\n", 23);
			LINK_PA_LED = ON;						// LINK_PA指示灯点亮

			if(pa_state_chk() == 0)					// PA状态线都没有拉低，则进入LINK_PA到PA状态
			{
				link_pa_2_pa_on();					// LINK_PA到PA通道打开

				PA_LED = ON;						// PA指示灯点亮

				debug_printf("work_status = LINK_PA_2_PA_SUCCESS\r\n", 36);
				work_status = LINK_PA_2_PA_SUCCESS;	// 工作状态为LINK_PA到PA
			}
		}
	}
	else if(work_status == LINK_PA_2_PA_SUCCESS)	// 工作状态为LINK_PA到PA状态
	{
		link_pa_led_blink(50);						// LINK_PA_LED指示灯间隔500ms闪亮

		if(link_pa_state_chk() == 0)				// 检测到LINK_PA电平抬高，LINK_PA_STATE_CHK = 1
		{
			debug_printf("LINK_PA_STATE_CHK = 0\r\n", 23);
			debug_printf("work_status = LINK_PA_2_PA_SUCCESS\r\n", 36);
...
void link_pc_2_pc(void)
{
	if((work_status == IDLE) || (work_status == PA_2_LINK_PA_SUCCESS) || (work_status == LINK_PA_2_PA_SUCCESS))		// 工作在空闲、PA到LINK_PA、LINK_PA到PA状态， PC优先级高于PA
	{
		if(link_pc_state_chk() == 1)				// 检测到LINK_PC状态拉低，LINK_PC_STATE_CHK = 0
		{
			...
			if(pc_state_chk() == 0)					// PC状态线都没有拉低，则进入LINK_PC到PC状态
			{
				link_pc_2_pc_on();					// LINK_PC到PC通道打开

				PC_LED = ON;						// PC指示灯点亮

				debug_printf("work_status = LINK_PC_2_PC_SUCCESS\r\n", 36);
				work_status = LINK_PC_2_PC_SUCCESS;	// 工作状态处于LINK_PC到PC状态
			}	
		}
	}
	else if(work_status == LINK_PC_2_PC_SUCCESS)	// 工作状态为LINK_PC到PC状态
	{
		link_pc_led_blink(50);						// LINK_PC_LED指示灯间隔500ms闪亮

		if(link_pc_state_chk() == 0)				// 检测到LINK_PC电平抬高，LINK_PC_STATE_CHK = 1
		{
			debug_printf("LINK_PC_STATE_CHK = 1\r\n", 23);
			debug_printf("work_status = LINK_PC_2_PC_SUCCESS\r\n", 36);

			link_pc_2_pc_off();						// LINK_PC到PC通道关闭
...
void pa_2_link_pa_on(void)					// PA到LINK_PA通道打开
{
	...
	PA_AUDIO_IN = ON;								// PA输入打开
					
	debug_printf("PA_AUDIO_OUT = OFF\r\n", 20);
	PA_AUDIO_OUT = OFF;								// PA输出关闭
						
	debug_printf("LINK_AUDIO_IN = OFF\r\n", 21);
	LINK_AUDIO_IN = OFF;							// LINK_AUDIO输出关闭
					
	debug_printf("LINK_AUDIO_OUT = ON\r\n", 21);
	LINK_AUDIO_OUT = ON;							// LINK_AUDIO输出打开
			
	debug_printf("LINK_PA_CTRL = ON\r\n", 19);
	link_pa_ctrl(ON);								// 拉低LINK_PA状态线
	delay_1ms(50);									// 延时50ms使LINK_PA状态稳定
}
					
void pa_2_link_pa_off(void)					// PA到LINK_PA通道关闭
{
	debug_printf("PA_AUDIO_IN = OFF\r\n", 19);
	PA_AUDIO_IN = OFF;								// PA输入关闭
	...
	LINK_AUDIO_OUT = OFF;							// LINK_AUDIO输出关闭
			
	debug_printf("LINK_PA_CTRL = OFF\r\n", 20);
	link_pa_ctrl(OFF);								// 抬高LINK_PA状态线
	delay_1ms(50);									// 延时50ms使LINK_PA状态稳定
}
...
void pc_2_link_pc_on(void)					// PC到LINK_PC通道打开
{
	...
	PC_AUDIO_IN = ON;								// PC输入打开

	debug_printf("PC_AUDIO_OUT = OFF\r\n", 20);
	PC_AUDIO_OUT = OFF;								// PC输出关闭

	debug_printf("LINK_AUDIO_IN = OFF\r\n", 21);
	LINK_AUDIO_IN = OFF;							// LINK_AUDIO输入关闭

	debug_printf("LINK_AUDIO_OUT = ON\r\n", 21);
	LINK_AUDIO_OUT = ON;							// LINK_AUDIO输出打开

	debug_printf("LINK_PA_CTRL = ON\r\n", 19);
	link_pc_ctrl(ON);								// 拉低LINK_PC态线
	delay_1ms(50);									// 延时50ms使LINK_PC状态稳定
}

void pc_2_link_pc_off(void)					// PC到LINK_PC通道关闭
{
	debug_printf("PC_AUDIO_IN = OFF\r\n", 19);
	PC_AUDIO_IN = OFF;								// PC输入关闭
...
void link_pa_2_pa_on(void)					// LINK_PA到PA通道打开
{
	...
	PA_AUDIO_OUT = ON;							// PA输出打开

	debug_printf("LINK_AUDIO_IN = ON\r\n", 20);
	LINK_AUDIO_IN = ON;							// LINK_AUDIO输入打开
...
void get_ver_info(void)
{
	...
	put_n_char("\t版本信息:  Ver 1.1  2014-03-29\r\n", 33);
